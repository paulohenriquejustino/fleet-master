...
class ChecklistCaminhaoScreen extends StatefulWidget {
  ...
  @override
  State<ChecklistCaminhaoScreen> createState() => _ChecklistCaminhaoScreenState();
}
...
class _ChecklistCaminhaoScreenState extends State<ChecklistCaminhaoScreen> {
  ...
  Future<void> fetchPlacasDisponiveis() async {
    ...
    for (var doc in snapshot.docs) {
      ...
      if (placa != null && modelo != null && cidade != null) {
        fetchedplacas.add({'placa': placa, 'modelo': modelo, 'cidade': cidade});
      }
    }
      // Atualizando a tela.
      setState(() {
  ...
  if (selectedPlaca != null) {
    ...
    if (checklistExiste) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Checklist já foi feito para essa placa!')),
      );
    } else {
...
Future<bool> verificarSeChecklistJaExiste(String placa) async {
  try {
    // Buscar documentos na coleção 'checklist_caminhao' onde a placa é igual à placa selecionada
    QuerySnapshot snapshot = await FirebaseFirestore.instance
        .collection('checklist_caminhao')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Checklist de Caminhão'),
        centerTitle: true,
      ),
      body: Column(
        children: [
        ...
        child: Column(
          ...
          children: [
            TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Km de início'),
    ...
    return Column(
      ...
      children: [
        ...
        TextField(
          ...
          onChanged: (value) {
            setState(() {
              items[itemIndex].nonConformityDescription = value;
            });
          },
        ),
      ],
...
Future<void> addChecklistToFirebase(
  ...
  final checklistCaminhao =
      FirebaseFirestore.instance.collection('checklist_caminhao');
